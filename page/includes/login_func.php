<?php
function register($name, $email, $pas) { // регистрация
    $name = trim($name);
    $email = trim($email);
    $pas = trim($pas);
    $passl = strlen($pas);
...
function login($name, $pas) { // вход в аккаунт
    $name = trim($name);
    $pas = trim($pas);
    $passl = strlen($pas);
    $pas = md5($pas);
...
function chengePassword($lastPass, $newPass, $newPass2) { // смена пароля
...
function chengeEmail($email, $pass) { // сменя почты
    $email = trim($email); 
    $pass = trim($pass);
    $emaill = strlen($email);
    if(1 > $emaill || $emaill >= 31) {
